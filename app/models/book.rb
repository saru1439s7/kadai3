...
##############################################################
  ...
  def create_notification_book_comment!(current_user, book_comment_id)
    ...
    temp_ids = Bookomment.select(:user_id).where(book_id: id).where.not(user_id: current_user.id).distinct
  ...
  def save_notification_bookcomment!(current_user, book_comment_id, visited_id)
